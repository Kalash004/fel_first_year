...
void do_cycle(Array *primes, int *stop_flag)
{
    ...
    int loop = 0;
    do {
        if (loop == max_factor_id)
            break; // End of printing

        int factor = get_next_factor(primes, &input, &loop);
        ...
        if (loop != 1 && loop < max_factor_id && factor != last_factor)
            printf(" x ");
